...
export default function CustomerLeaderboardPage() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const { data } = await api.get('/leaderboard/customer');
        if (data.success) {
  ...
  const isInTop10 = myStats && top.some((c: any) => c._id === myStats._id);
    ...
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs font-medium text-gray-400">{user?.name}</p>
              </div>
            </div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 border-b border-white/[0.06] flex items-center gap-2">
          ...
          <h2 className="font-semibold text-white">Top 10 Customers</h2>
        </div>
        {top.length === 0 ? (
          <div className="text-center py-12 text-gray-500">
            <Trophy className="w-10 h-10 mx-auto mb-2 opacity-20" />
            <p className="text-sm">No customers ranked yet — complete an order to be the first!</p>
          </div>
        ) : (
